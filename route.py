...
def update(inc_num):
    if request.method == "GET":
        ...
        db = c.db_connect()
        col = db['pipeline']
        results = col.find_one({"Incident Number": inc_num}, {'Incident Number': 1, 'Incident Types': 1, 'Reported Date': 1, 'Nearest Populated Centre': 1,
                   'Province': 1, 'Company': 1, 'Substance': 1, 'Significant': 1, 'What happened category': 1,
                   '_id': 0})
...
def chart():
    ...
    fig1 = px.bar(df, x='Significant', y='Province', color='Incident Number',
                 barmode='group')
    graph1JSON = json.dumps(fig1, cls=plotly.utils.PlotlyJSONEncoder)
    header1="Fruit in North America"
    description1 = ""

    fig2 = px.pie(df, values='Reported Date', names='Substance', title='Population of European continent')
    graph2JSON = json.dumps(fig2, cls=plotly.utils.PlotlyJSONEncoder)

    colors = ['gold', 'mediumturquoise', 'darkorange', 'lightgreen']

    fig3 = go.Figure(data=[go.Pie(labels=['Oxygen', 'Hydrogen', 'Carbon_Dioxide', 'Nitrogen'],
                                 values=[4500, 2500, 1053, 500])])
    fig3.update_traces(hoverinfo='label+percent', textinfo='value', textfont_size=20,
                      marker=dict(colors=colors, line=dict(color='#000000', width=2)))
    graph3JSON = json.dumps(fig3, cls=plotly.utils.PlotlyJSONEncoder)
    return render_template("chart.html", title="Chart", graph1JSON=graph1JSON, header1=header1, description1=description1, graph2JSON=graph2JSON, graph3JSON=graph3JSON)
